...
bool gearUp() {
    ...
    //  Shift from 1st to Neutral ( use manual clutch )
    ...
    //  Shift up from neutral to 2nd ( use clutch )
    else if ( currentGear == 0 )
    {
        //  Get clutch pressure
        clutchPressure = getClutchPressure();
...
bool gearDown() {
    ...
    if ( clutchPressure > minClutchPressureShift ) 
    {
        ...
        //  If in first gear, do nothing
        else if ( currentGear == 1 ) 
        {
            //  Do Nothing
        } 
        //  If in second gear, go to neutral ( down half step, use clutch )
