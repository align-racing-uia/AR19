...
void setup() {
  ...
  launchButtonsMSG.can_dlc  = 2; //? message length TBD
...
//  mcp2515.sendMessage(&debuggingMSG);
  ...
  if ( IsBothButtonsPressed() && !bLaunchActive) //if both buttons are pressed AND Launch is currently not active/regulating, check if both buttons are held
  {
     SetLEDColor('g'); //Green
        ...
        if (currentSystemState == 2) //Check if the current message has the correct ID and the desiret data
          {
  ...
  if (bLaunchActive)  //if both buttons are confirmed held and Launch is currently active,
  {
    ...
    if ( IsOneButtonNotPressed() ) //if one of the buttons are released,
      {
      MakeTimeStamp();
      if ( ButtonsReleasedWithinTime() && IsNoButtonsPressed() && (currentSystemState == 3) ) //If dt < 500ms AND both buttons are released AND the vehicle is ready to launch
        {
        //Confirmed Take-off
