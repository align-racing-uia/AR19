...
// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! //
  ...
  //Pin number setup
  const int iButtonPins[4] = {A4, A5, 0,0};// 11, 12}; //?Add ocrrect button pin numbers - In order: Left button, Right Button, Left Flappy, Right Flappy
...
void setup() {
  ...
  launchButtonsMSG.can_dlc = 4; //? message length TBD
...
//  mcp2515.sendMessage(&debuggingMSG);
  ...
  if ( IsBothButtonsPressed() && !bLaunchActive) //if both buttons are pressed AND Launch is currently not active/regulating, check if both buttons are held
  {
     SetLEDColor('g'); //Green
        ...
        if ((IncMessageID == 0x2A0) && (IncMessageLocal[0] == 2)) //Check if the current message has the correct ID and the desiret data
          {
...
void UpdateFlappies()
{
  ...
  outgoingMSG.can_id = 0x240; //Set adress ???
  outgoingMSG.can_dlc = 1;
  outgoingMSG.data[0] = 1;
  ...
  outgoingMSG.can_id = 0x240; //Set adress ???
  outgoingMSG.can_dlc = 1;
  outgoingMSG.data[0] = 0xFF;
  mcp2515.sendMessage(&outgoingMSG);

  }
  else
  {
  outgoingMSG.data[0] = 0;  
  }
...
void SendRotarySwitchData(uint8_t dataInput)
{
  ...
  {
    outgoingMSG.can_id = 0x235;
    outgoingMSG.can_dlc = 1;
    outgoingMSG.data[0] = dataInput;
    mcp2515.sendMessage(&outgoingMSG);
