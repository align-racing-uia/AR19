...
//  Current System State made Global Variable
  ...
  //Pin number setup
  const int iButtonPins[4] = {A4, A5, A2, 3}; // 3, 10}; //?Add ocrrect button pin numbers - In order: Left button, Right Button, Left Flappy, Right Flappy
...
void CheckButtons() 
  {
    for (int i = 0; i < 4; i++)
    {
      bButtonValues[i] = !digitalRead(iButtonPins[i]); //inverse due to internal pull-up
    }
  }
