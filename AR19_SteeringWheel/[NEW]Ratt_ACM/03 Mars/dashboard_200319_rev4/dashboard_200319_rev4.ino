...
// FAILURE DETECTED: LAUNCH CONFORMATION FAILED //
...
void loop() {
  ...
  mcp2515.readMessage(&incommingMSG);
          ...
          if ((incommingMSG.can_id == 0xE0) && (incommingMSG.data[0] = 0x01)) //Check if the current message has the correct ID and the desiret data
          {
          bLaunchActive = 0; //?if and ONLY if confirmed with LB
          analogWrite(5, 255);
          analogWrite(6, 255);
          analogWrite(9, 255);
          Serial.print("Reset");
 ...
 //       {
          if ((incommingMSG.can_id == 0xAD) && (incommingMSG.data[0] = 0xFF)) //Check if the current message has the correct ID and the desiret data
          {
