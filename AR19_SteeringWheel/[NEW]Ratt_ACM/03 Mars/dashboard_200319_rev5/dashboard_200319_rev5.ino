...
void setup() {
  ...
  canMSG1.can_id = 0xF0; //? id TBD, confirmation request from dash to LB
...
//  mcp2515.sendMessage(&debuggingMSG);
  ...
  if ( IsBothButtonsPressed() && !bLaunchActive) //if both buttons are pressed AND Launch is currently not active/regulating, check if both buttons are held
  {
        ...
        {
          if ((incommingMSG.can_id == 0x5C1) && (incommingMSG.data[0] == 2)) //Check if the current message has the correct ID and the desiret data
          {
