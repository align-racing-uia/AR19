...
void loop() {
  ...
  if ( IsBothButtonsPressed() && !bLaunchActive) //if both buttons are pressed AND Launch is currently not active/regulating, check if both buttons are held
  {
        ...
        if ((IncMessageID == 0x5C2) && (IncMessageLocal[0] == 2)) //Check if the current message has the correct ID and the desiret data
          {
  ...
  if (bBothButtonsPressed && bLaunchActive)  //if both buttons are confirmed held and Launch is currently active,
  {
