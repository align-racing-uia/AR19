...
const uint16_t canIDspeed = 0x218;
const uint16_t canID = 0x220; // canID: 0x2F1 -> Sprocket, 0x2F0 -> Diff, 0x220 -> FL, 0x210 -> FR
double hallPPR = 24.0;


double rpm = 0.0, rpmAvg;
...
unsigned long timerSpeedSender = 0;
...
struct can_frame canMsg4, canMsgInn, canMsgSpeedOut;
...
void setup()
{
  ...
  canMsgSpeedOut.can_id = canIDspeed;
  canMsgSpeedOut.can_dlc = 1;
  canMsgSpeedOut.data[0] = 0x00;
...
void loop()
{
  ...
  if(millis() - timerSpeedSender > 100)
  {
    speedSend();
    timerSpeedSender = millis();
  }
...
void canRead(){
  ...
  if (mcp2515.readMessage(&canMsgInn) == MCP2515::ERROR_OK) {

    if(canMsgInn.can_id == 0x210){

        for (int ii = 0; ii < canMsgInn.can_dlc; ii++){

            data.arrNum[ii] = canMsgInn.data[ii];
...
void speedSend()
{
  rpmAvg = (rpm + (float) hallFR)/2;
  canMsgSpeedOut.data[0] = rpmAvg/1000.0f*3.14f/30.0f*0.2032f*3.6f;
  mcp2515.sendMessage(&canMsgSpeedOut);
}
